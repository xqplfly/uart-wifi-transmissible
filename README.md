...
- ✅ **锂电池供电支持**
- ✅ **电池电量监测**
- ✅ **多状态LED指示**（支持工作状态、连接状态、低电量、错误告警与指令反馈）
...
### LED功能说明
- **启动自检**：设备上电后，LED会先进行一次快速闪烁，用于提示系统正在初始化
- **模式指示**：通过不同颜色与闪烁频率区分客户端模式、服务器模式和连接状态
- **指令反馈**：收到串口、Web 或网络指令后，LED会短暂闪烁一次，表示系统已接收并处理
- **低功耗提示**：进入 Light Sleep 后，LED切换为呼吸灯效果，便于识别设备处于待机状态
- **异常告警**：当 SD 卡异常或电池电量过低时，LED会切换为高优先级告警颜色，提醒及时处理

### LED灯效定义
- **常亮**：表示状态稳定，无需额外关注
- **快闪**：表示状态变化频繁或异常需要立即注意
- **慢闪**：表示当前处于等待、未连接或低优先级提醒状态
- **呼吸灯**：表示低功耗或休眠相关状态
- **短闪**：表示一次性的事件确认，如收到命令或完成操作
